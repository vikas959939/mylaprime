...
const MyScheduler = lazy(() => import("./myComponent/Scheduler"))
const MyLocation = lazy(()=> import("./pages/app/view location/Location"))
...
const SetupData = lazy(() => import('./pages/app/Setup/Setupdata'))
const Freeslots = lazy(()=> import('./pages/app/view location/Freeslots'))
const Suplierlogistics = lazy(()=> import('./pages/app/Suplier logistics/Suplierlogistics'))
...
const enrollVlaue = () => {
  let myData = localStorage.getItem('enrollvalue');
  console.log(myData)

  if (myData) {
    return JSON.parse(localStorage.getItem('enrollvalue'));
  } else {
    return ''
  }
}
...
function App() {
  ...
  const [EnrollMentValue, setEnrollMentValue] = useState(enrollVlaue())
  const [loginData, setLoginData] = useState(getLocalItems())


  useEffect(() => {
    localStorage.setItem('enrollvalue', JSON.stringify(EnrollMentValue))
  }, [EnrollMentValue])
  ...
  return (
    ...
    <main className="App  relative">
      <Routes>
        ...
        <Route path="/*" element={<Layout  loginData={loginData} setLoginData={setLoginData}/>}>
          ...
          <Route path="kanban" element={<KanbanPage  EnrollMentValue={EnrollMentValue}/>} />
          ...
          <Route path="setup" element={<SetupData setEnrollMentValue={setEnrollMentValue}/>} />
          <Route path="location" element={<MyLocation />} />
          <Route path="freeslots" element={<Freeslots />} />
          <Route path="suplier-logistics" element={<Suplierlogistics />} />
